...
impl Map
{
    ...
    pub fn new() -> Self
    {
        Map { cells: vec![Cell::new(); CELLS_PER_ROW * CELLS_PER_COL],
              mrows: CELLS_PER_ROW,
              mcols: CELLS_PER_COL,
    ...
    /// Runs the next generation of the automaton
    ...
    /// Returns which is the next state of a cell based on it's neighbours
    fn get_new_state(cell: &Cell, neighbours: Vec<&Cell>) -> CellState
    {
        match cell.state {
            ...
            CellState::Conductor => {
                let total_heads = neighbours.iter()
                                            .filter(|x| x.state == CellState::ElectronHead)
                                            .count();
    ...
    /// If some of the adjacent cells are not available, they will not be in the
    /// vector (e.g. the provided cell is on row 1 so there's no upper
    /// adjacent cells)
    pub fn get_neighbours(&self, x: usize, y: usize) -> Vec<&Cell>
    {
        ...
        vec![// (W) same row left
             ...
             self.get_cell(xpos + 1, ypos - 1),
             // (SE) lower row right
             self.get_cell(xpos + 1, ypos + 1),].into_iter()
                                                .filter_map(|e| e)
                                                .collect()
    }
}
    ...
    for x in 0..CELLS_PER_ROW {
        ...
        for y in 0..CELLS_PER_COL {
            ...
            canvas.draw_rect(rect)
                  .expect(&format!("couldn't draw line: ({},{}) -> ({},{})",
                                   0, y, SDL2_WIN_WIDTH, y));
        }
    }
